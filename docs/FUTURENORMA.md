...
Seven areas, one masthead, one context row that names the organization, the
role, the subscription state and the environment on every page. The map and the
matrix are **one list** in `src/consoleIA.ts` — the navigation renders from it,
every page guards from it, and the suite imports and runs it rather than reading
it. Five of the seven areas hold nothing yet and say so, listing what will be
there from the map itself. Built before the workflows because PATHWAYS §5 says
to, and the reason showed immediately: two pages that each built their own
chrome could not agree on anything the shell now owns.

**Two defects came out of it, and neither was failing anything.**
`/repos/{id}/trend` and its CSV export still answered only to `NORMA_DEV_OPEN`,
so every customer who clicked a sparkline got "Not found" — while `PATHWAYS.md`
§7 recorded that gate as closed. And the seed wrote `role: 'owner'` into
...
`npm run seed:real` loads ten runs that actually happened, with their real
images, percentages and recorded findings; and `npm run capture:cloud`
photographs every Cloud page in both themes. Suite counts are **955 on PGlite /
983 on real Postgres across thirty suites**. The migration range is unchanged at
`001`–`020` — none of this needed a column. §2 gains three rows and
`FinishedSPEC.md` §3w carries the evidence, including the two shipped bugs that
real data found: duplicate React keys on the diff overlay's region boxes, and
"first drifted at" reporting **"never exceeded the threshold" beside "Times
flagged: 4 runs"** because one field carried two facts.

**One thing here is a decision, already taken and recorded rather than built.**
A "generated by *username*" line was asked for in the report header. There is no
session layer until Step 6, so the only identities available today are an API
key label or a name the CLI sends — and **Harsha decided on 2026-08-20 to wait
for Step 6** rather than ship either. The settled policy for when it lands:
**both owner and share views, display name only, never an email address.**
`PATHWAYS.md` Pathway 5 carries it as a Step 6 work item.

Before that, **2026-08-20** — **Step 4 (trends) is built**: a repository view, a
frame trend chart, and an org-scoped trends API (BuildV5 Phase I). Suite counts
were **878 on PGlite / 906 on real Postgres across twenty-eight suites**.
...
needs Step 5. The real-run tenant does not change this: it is genuine data in a
local database, which is evidence about the *data* and not about the deployment.
