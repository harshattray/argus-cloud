...
carries a "?" that opens a plain-language definition from the same glossary the
public `/report` page prints; `npm run seed:real` loads ten runs that actually
happened, with their real images, percentages and recorded findings; and
`npm run capture:cloud` photographs every Cloud page in both themes. Suite counts
are **930 on PGlite / 958 on real Postgres across thirty suites**. The migration range is unchanged at
`001`–`020` — none of this needed a column. §2 gains three rows and
`FinishedSPEC.md` §3w carries the evidence.
...
| **Every Cloud figure explains itself** | ✅ 2026-08-20 | A "?" beside each number opens a definition — 103 on a seven-frame report. The text comes from `web/lib/glossary.ts`, which the public `/report` page also prints, so the words a prospect reads before signing up are the words they read after. Native HTML popover, so `/repos/` still ships **zero client JavaScript** and no new inline styles. 31 checks — `FinishedSPEC.md` §3w |
...
(§4h). Full suite:
**930 checks green** on PGlite across thirty suites — `apiKeyRevocation`,
...
`waitlistConfirmationEmail`, `webhooks` — and **958** against a real Postgres
