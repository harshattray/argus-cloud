...
6. Billing via MoR (sandbox-evaluate Paddle vs Lemon Squeezy, pick one): checkout, webhooks → plan state, ~~14-day no-card trial~~ (**superseded 2026-08-03 — there is no trial; see BuildV5.md §G2c for the decision, the reasoning, and the deferred re-enable design. `plan` is `free | team | lapsed`; risk reversal is a 30-day money-back guarantee. The "trial expiry" case in this stage's billing test plan is void.**), 14-day lapse grace — **uploads rejected politely on lapse, CI stays green, nothing deleted on lapse**. Plan limits as config, not code: Team = 10 repos, unlimited seats; the price ladder runs on repos (value axis), never on seats (distribution axis); an org exceeding limits gets a clear upgrade prompt, and a very large org on a starter plan is flagged in the admin view as an enterprise lead, not treated as abuse.
...
| Billing (sandbox) | checkout→webhook→active **+ org provisioned**; cancel→grace at both boundaries (simulated clock); tampered webhook signature; refund→plan lapses, nothing deleted (~~trial expiry~~ — void, no trial) | State machine exact; tampered payload rejected |
