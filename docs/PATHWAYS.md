...
| 6 | `style-src-attr 'unsafe-inline'` is unchanged, and the explainers did not widen it | Recorded because it was the live question when they were built. The bubbles position from `_styles/surface.module.css` and CSS anchor positioning off the popover's *implicit* anchor — an `anchor-name` would have to be unique per instance and could only come from an inline style. Carried-forward item 2 stands exactly where it did. |

**Closed on this pathway — 2026-08-20.** Every figure on `/r/` and `/repos/` now
opens a plain-language definition from `web/lib/glossary.ts`, the same file the
public `/report` page prints, so the vocabulary a prospect learns before signing
up is the vocabulary the product uses after. Native HTML popover: `/repos/` still
ships zero client JavaScript. `npm run seed:real` adds a second tenant of ten
runs that actually happened — real images, real percentages, the real recorded
findings — kept in its own organization so "(sample data)" is never stamped on a
measurement, and `npm run capture:cloud` photographs every page in both themes
into `docs/screenshots/cloud/`. `FinishedSPEC.md` §3w, 52 checks across two new
suites.
