...
> invitation *email*, the "generated by" line, and identity linking from an
> account page.
>
> **What "not done" means precisely, recorded 2026-08-22 so the next piece of
> work is not mis-scoped.** The lifecycles behind those pages are *written and
> under test* — `src/invitations.ts` has create, accept, revoke, list and expire
> with the hashed single-use token and the conditional consume; `src/apiKeys.ts`
> has create, list, revoke and per-key budgets; `src/ownerClaims.ts` has the
> claim. **None of them has an HTTP route or a page.** `web/app/api/` holds
> `auth`, `blob`, `ci-explain`, `explain`, `org`, `share`, `trends`, `upload`,
> `waitlist`, `webhooks` and the unlock routes, and nothing else. An admin
> cannot invite a colleague or mint an upload key today without a database
> client. **So the remaining Step 6 work is mostly a browser surface over
> services that exist, not new domain logic** — which is also why the surface
> is where the risk now sits: every one of those services is an admin-only
> write, and 5A.9's matrix has to hold on the route, not in the navigation.
>
...
| # | Work | Why here |
|---|---|---|
| 1 | **Organization area** — members, roles, invitations, API and agent keys | The only remaining area whose whole data layer is already written (5A.6, 5A.10). It is also the one that unblocks the most of 5A.12's gate list: invitation states, membership removal taking effect on the next request, and key isolation across tenants |
...
**Item 1's gate:** against real Postgres, over HTTP, on a production build with
`NORMA_DEV_OPEN=0` — an invitation moves `pending → accepted` exactly once under
two concurrent consumptions; a resend leaves exactly one live token; a member
and a designer are refused every write; an admin of organization B cannot see,
create or revoke anything belonging to organization A; a revoked key fails
closed on the next request with no cache extending its life; a created key is
shown once and is not retrievable afterwards; and removing the last admin is
refused. Each watched failing before it is believed (CLAUDE.md rule 3).
