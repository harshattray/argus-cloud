...
| 7 | Chart tooltips need a pointer, so a touch screen gets nothing extra | Hover is an enhancement, not the only route: every point is also a row in the Runs table with the same facts and a link. A tap-to-open card would need either client JavaScript on a tree that has none, or a `<details>` per point — neither earns its place before there is a customer on a tablet. |
