...
>
> **2026-08-23 completes 5A.8's browser surface. The account page is built.**
> `/account` lists every live session for one person — device label, sign-in
> method, started, last used, expiry, a current-browser marker and a per-row
> sign-out — with their memberships and their own sign-in history beside it.
> 5A.14 item 2 is done and `FinishedSPEC.md` §3ag is the evidence.
>
> **It is a fourth kind of surface, and the map now says so.** Not one of the
> seven areas: user-scoped, no membership required, no role to refuse.
> `consoleIA.ts` gained a `Surface` type that `ConsoleArea` extends, and
> `ACCOUNT_SURFACE` sits beside `CONSOLE_AREAS`. The alternative — letting the
> page through S11.11 by naming it in the test's exception list — would have put
> a structural decision in a file nobody reviews as one.
>
> **Two defects it exposed, neither failing anything.** `listSessions` filtered
> on revoked and expired but not on the idle cutoff, so a browser unused for 31
> days was listed as signed in while `resolveSession` would already refuse it.
> And the device column would have rendered the stored user agent, which is a
> client-chosen string of up to 300 characters; it renders one of a fixed set of
> our own phrases instead.
>
> **The scope lesson from the Organization area, applied before it could bite.**
> `revokeSession` took an id and nothing else. That was safe while the only
> caller passed an id it had just resolved from a cookie, and stopped being safe
> the moment a form supplied one — so it takes a required user scope, `null`
> meaning "any", exactly as `revokeApiKey` takes an organization. `G7` proves it
> over HTTP with two colleagues in **one** organization, which is the case a
> tenant boundary would not have caught.
>
> Before that, **2026-08-22 added two rows of 5A.8 without the page they belong
> to**: *sign out* and *sign out everywhere*, in an account menu on every
> signed-in page.
>
...
> restated. The **individual account dashboard was not built** at that point; it
> was the natural next piece and became 5A.14 item 2, above.
>
...
>
> **Not done:** the operator console, the billing page, deletion UI, privacy
> controls, the "generated by" line, identity linking, leaving an organization
> from the account page, and `api_keys.last_used_at` — deferred deliberately
> because `findApiKey` runs on every authenticated request, so recording last
> use is a write on the hot path. The account page itself, and the session list
> on it, landed 2026-08-23.
>
...
| 2 | ~~**Account page** — 5A.8's session list, with device label, method, last-seen, current-session marker and per-row revoke~~ ✅ **done 2026-08-23** | Explicitly owed: the masthead menu *ended* sessions and deliberately did not *show* them, so the person could not see what they were ending. All five columns, plus identity, memberships and the person's own sign-in history. `FinishedSPEC.md` §3ag |
| 3 | ~~**The refusal status decision** — FUTURENORMA §4 Open decisions 5~~ ✅ **done 2026-08-23** | It got dearer with every page added, and items 1 and 2 added several. Approved as recommended: `notFound()` and a real 404 across all three page families, sentences unchanged. The blocker was a `loading.tsx` per route streaming the response before a status could be set — `FinishedSPEC.md` §3ah |
